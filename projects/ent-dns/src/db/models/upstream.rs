...
impl UpstreamRepository {
    pub async fn list(pool: &SqlitePool) -> Result<Vec<Upstream>> {
        let rows = query_as::<_, Upstream>(
            "SELECT * FROM dns_upstreams ORDER BY priority ASC, name ASC"
        )
        .fetch_all(pool)
        .await?;
        Ok(rows)
    ...
    pub async fn get(pool: &SqlitePool, id: &str) -> Result<Option<Upstream>> {
        let row = query_as::<_, Upstream>(
            "SELECT * FROM dns_upstreams WHERE id = ?"
        )
        .bind(id)
        .fetch_optional(pool)
        .await?;
    ...
    pub async fn update(pool: &SqlitePool, id: &str, req: UpdateUpstream) -> Result<Upstream> {
        ...
        q.fetch_one(pool).await.map_err(|e| anyhow::anyhow!("Failed to update upstream: {}", e))
    }
