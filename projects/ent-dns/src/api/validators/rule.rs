...
mod tests {
    ...
    fn test_valid_filter_rules() {
        ...
        assert!(validator.validate_rule("filter", "||ads.example.com^").is_ok());

        // White list format
        assert!(validator.validate_rule("filter", "@@||example.com^").is_ok());

        // Hosts format
        assert!(validator.validate_rule("filter", "0.0.0.0 example.com").is_ok());
        assert!(validator.validate_rule("filter", "127.0.0.1 ads.example.com").is_ok());
    ...
    fn test_valid_rewrite_rules() {
        ...
        assert!(validator.validate_rule("rewrite", "myapp.local -> 192.168.1.100").is_ok());
        assert!(validator.validate_rule("rewrite", "example.com -> ::1").is_ok());
        assert!(validator.validate_rule("rewrite", "  myapp.local  ->  192.168.1.100  ").is_ok());
    }
    ...
    fn test_invalid_rewrite_format() {
        ...
        // Missing arrow
        let err = validator.validate_rule("rewrite", "myapp.local 192.168.1.100").unwrap_err();
        assert_eq!(err.code, "E009");

        // Multiple arrows
        let err = validator.validate_rule("rewrite", "myapp.local -> IP -> 192.168.1.100").unwrap_err();
        assert_eq!(err.code, "E009");
    }
    ...
    fn test_invalid_rewrite_parts() {
        ...
        // Empty domain
        let err = validator.validate_rule("rewrite", " -> 192.168.1.100").unwrap_err();
        assert_eq!(err.code, "E010");

        // Empty IP
        let err = validator.validate_rule("rewrite", "myapp.local -> ").unwrap_err();
        assert_eq!(err.code, "E010");
    }
    ...
    fn test_invalid_rule_type() {
        let validator = RuleValidator::new();
        let err = validator.validate_rule("unknown", "example.com").unwrap_err();
    ...
    fn test_invalid_ip_in_hosts_format() {
        let validator = RuleValidator::new();
        assert!(validator.validate_rule("filter", "999.999.999.999 example.com").is_err());
    }
}
